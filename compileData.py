...
def print_general(total_set):
    ...
    assignDate(total_set)
    assignCity(total_set)
    genders = count_by("GENERO", total_set)
    avg_gpa =  calc_dics_avg("GPA", total_set)
    years = count_by("year", total_set)
    dest_campus =  count_by("CAMPUS", total_set)
    places = count_by("city", total_set)
    perc_male = round(float(genders["masculino"]) / total_length * 100, 2)
    perc_female = round(float(genders["femenino"]) / total_length* 100, 2)
    ...
    sorted_places = sortByValue(places)
        ...
        #printing
    print "Total students: %d" % (total_length)
    print "Percentage of Male students:", perc_male, "%"
    print "Percentage Female students: ", perc_female, "%"
    print "Average GPA:", avg_gpa
    ...
    print "Cities of Origin:"
    printDic(sorted_places, places, 10, True)
    print "\n\nMajor Specific Data Below:\n"


def main ():
    data_file = load_data(sys.argv[1])
    everyone = extract_by("Location 1", "pr", data_file)
    print_general(everyone)
